...
class ProductLabelReportCustom2(models.AbstractModel):
    ...
    # we needed to pass ids because reports only accepts native python types (int, float, strings, ...)
        ...
        for product in products:
            ...
            production = env['mrp.production'].search([('product_id', '=', product.id)], order='date_start desc', limit=1)
            production_data[product.id] = production.lot_producing_id if production.lot_producing_id else False
            total += q
        ...
        return {
            'quantity': quantity_by_product,
            'production_data': production_data,  # Agregamos los datos de producción
